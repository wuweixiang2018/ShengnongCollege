...
//    protected TXLivePushListenerImpl mTXLivePushListener;
    ...
    public void login(final IMMessageMgr.Callback callback) {
        mSelfAccountInfo = new SelfAccountInfo(BaseUtil.userData.getMobile(), BaseUtil.userData.getRealName(),
                BaseUtil.userData.getPhotograph(), BaseUtil.userSig, BaseUtil.IM_ACCOUNT_TYPE,
                BaseUtil.IM_APP_ID);

        // 初始化IM SDK，内部完成login
        if (mIMMessageMgr != null) {
            mIMMessageMgr.initialize(mSelfAccountInfo.userID, mSelfAccountInfo.userSig,
                    (int) mSelfAccountInfo.sdkAppID, callback);
        }
    ...
    public void sendRoomCustomMsg(@NonNull String cmd, @NonNull String message, final IMMessageMgr.Callback callback) {
        ...
        String content = new Gson().toJson(customMessage, new TypeToken<CommonJson<CustomMessage>>() {
        }.getType());
...
//    }
    ...
    protected class HeartBeatThread extends HandlerThread {
        ...
        private Runnable heartBeatRunnable = new Runnable() {
            @Override
            public void run() {
                if (mSelfAccountInfo != null && mSelfAccountInfo.userID != null && mSelfAccountInfo.userID.length() > 0 ) {
//                        && mCurrRoomID != null && mCurrRoomID.length() > 0) {
